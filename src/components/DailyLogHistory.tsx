...
import { useDailyLogsQuery, useDeleteDailyLog, canEditOrDeleteLog } from "../hooks/useDailyLogs";
...
export const DailyLogHistory: React.FC<DailyLogHistoryProps> = ({ projectId, taskId }) => {
  ...
  if (isLoading) {
    return <div className="p-4 text-center text-ink-muted animate-pulse">Loading history...</div>;
  }
  ...
  if (logs.length === 0) {
    return (
      <div className="p-8 text-center bg-panel border border-divider rounded-2xl">
        ...
        <p className="text-[10px] text-ink-muted/70 mt-1">Log the first day of work to track progress.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {logs.map(log => (
        <div key={log.id} className="bg-surface p-5 rounded-2xl border border-divider shadow-sm relative overflow-hidden">
          {log.markComplete ? (
            <div className="absolute top-0 right-0 border-b-[32px] border-l-[32px] border-b-transparent border-l-emerald-500 w-0 h-0" title="Marked Complete">
            </div>
          ) : null}

          <div className="flex justify-between items-start mb-3">
             <div>
                <span className="text-sm font-black text-ink block">{format(new Date(log.workDate), "MMM d, yyyy")}</span>
             ...
             <div className="text-right">
                <span className="text-2xl font-black text-indigo-600 font-mono tracking-tighter leading-none">{log.progressPercent}%</span>
                <span className="text-[9px] font-bold text-ink-muted uppercase tracking-widest block text-right mt-1">Cum. Progress</span>
             </div>
          </div>
                 ...
                 <div>
                    ...
                    <ul className="space-y-1">
                      {log.materials.map((m, i) => (
                        <li key={i} className="text-[11px] font-bold text-ink/80 flex justify-between">
                           <span className="truncate pr-2">{m.name}</span>
                           <span className="font-mono">{m.quantity} <span className="text-[9px] text-ink-muted">{m.unit}</span></span>
                        </li>
                      ))}
                    </ul>
                 </div>
          ...
          {log.note && (
             <div className="mt-4 bg-amber-50/50 p-3 rounded-xl border border-amber-100 flex gap-2 items-start text-amber-900">
                ...
                <p className="text-[11px] font-medium leading-relaxed italic">{log.note}</p>
             </div>
          )}
