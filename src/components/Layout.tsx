...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const SidebarContent = ({ showLabels = true }) => (
    <div className="flex flex-col h-full">
      ...
      >
        <div className="bg-primary p-2 md:p-3 rounded-[12px] md:rounded-[16px] shadow-2xl shadow-[#A3711C]/30 flex items-center justify-center shrink-0">
          <Construction
        ...
        {menuItems.map((item) => (
          <button
            ...
            className={`flex items-center apple-transition group ${
              ...
              activeTab === item.id
                ? "bg-primary text-white shadow-xl shadow-[#A3711C]/20 ring-1 ring-primary/50"
  ...
  return (
    ...
    >
      {/* Desktop Sidebar */}
      <aside
        className={`hidden md:flex apple-glass rounded-[32px] transition-all duration-700 ease-[cubic-bezier(0.2,0,0,1)] flex-col shadow-2xl relative z-20 ${isSidebarOpen ? "w-[280px] xl:w-80" : "w-24 overflow-hidden"} ${uiMode === "site" ? "!rounded-none !shadow-none border-r border-[#E5E5EA] !bg-surface" : ""}`}
      >
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 bg-black/40 backdrop-blur-md z-[100] md:hidden animate-in fade-in duration-300"
      ...
      >
        {/* Top Bar Navigation */}
        <header
          className={`bg-surface/80 sm:bg-transparent sm:apple-glass border-b border-black/5 sm:border-0 rounded-none sm:rounded-[24px] px-4 py-3 sm:px-6 sm:py-4 lg:px-8 lg:py-5 flex items-center justify-between z-20 shrink-0 ${uiMode === "site" ? "!rounded-none !bg-white !border-b !border-[#E5E5EA] !py-3" : ""}`}
        >
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div
              className={`hidden sm:block h-6 w-px bg-surface/30 mx-1 lg:mx-2 ${uiMode === "site" ? "!bg-[#E5E5EA]" : ""}`}
            />
        ...
        <nav
          className={`fixed md:hidden bottom-0 left-0 right-0 bg-surface/90 backdrop-blur-xl border-t border-black/5 pb-[env(safe-area-inset-bottom)] pt-2 px-2 z-[60] flex justify-around items-center ${uiMode === "site" ? "!bg-white !border-[#E5E5EA]" : ""}`}
        >
