...
exports.pickMaterial = pickMaterial;
exports.askMaterialQty = askMaterialQty;
exports.pickLabourRole = pickLabourRole;
...
async function showMenu(tg, chatId, messageId, session) {
    ...
    await (0, session_1.setSession)(chatId, { step: "log:menu" });
    if (messageId)
...
async function pickMaterial(tg, chatId, messageId, session) {
    ...
    await (0, session_1.setSession)(chatId, { step: "log:material_pick" });
...
async function pickLabourRole(tg, chatId, messageId, session) {
    ...
    rows.push([{ text: "◀ Back", callback_data: "bk" }]);
    await (0, session_1.setSession)(chatId, { step: "log:labour_pick" });
...
async function handlePhoto(tg, chatId, session, photoSizes) {
    const d = session.draft || {};
    if (!d.logId) {
    ...
    await (0, session_1.setSession)(chatId, { draft: { ...d, photoUrls } });
    const s = await (0, session_1.getSession)(chatId);
    await showMenu(tg, chatId, null, s);
}
async function saveLog(tg, chatId, messageId, session) {
    const d = session.draft || {};
    const base = projPath(session.orgId, session.activeProjectId);
    ...
    await (0, session_1.setSession)(chatId, { recentTaskIds: recent });
