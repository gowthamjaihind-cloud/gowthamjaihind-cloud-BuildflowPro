...
// Reads a GST vendor invoice (image or PDF) with Gemini vision, then matches it
// to the project's vendors and open POs and flags discrepancies. Returns a
// draft VendorBill for the client to review and confirm — nothing is written.
export const extractVendorInvoice = onCall(
  ...
  async (request) => {
    ...
    // Company GST identity (for state-code / tax-split reasoning).
    let companyState = "";
    let companyGstin = "";
    if (orgId) {
    ...
    const raw = await callGeminiVision(fileBase64, mimeType, GEMINI_API_KEY.value());
    ...
    const lineItems = (raw.lineItems || []).map((li: any) => {
      ...
      // Trust the invoice's own tax numbers if present; else derive from state.
