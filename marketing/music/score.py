...
# The dynamic shapes live in a module with no numpy/scipy import, so their
# tests can run on a machine that has neither. See arcs.py.
from arcs import arc, walk_arc
...
def walkthrough(total, bpm=72, bloom=None):
    """
    A minor, no drums.
    ...
    bar = 4 * beat
    t = Track(total)
    # The lift is placed against where the narration stops, in seconds, and
    # starts LEAD before it so the duck is still masking the rise. RISE is how
    # long it takes to open up. Both fall back to a late guess when the caller
    # does not know where the voice ends.
    LEAD, RISE = 1.5, 2.3
    voice_end = bloom if bloom else total * 0.97
    bloom_at = min(max((voice_end - LEAD) / total, 0.70), 0.995)
    ramp_at = RISE / total
    tail_from = total * bloom_at
    progression = [
        (["A3", "C4", "E4"], "A2"),
        (["F3", "A3", "C4"], "F2"),
        (["C4", "E4", "G4"], "C3"),
    ...
    for b in range(n_bars):
        ...
        if at > total:
            break
        # Stop the loop turning over once the sustained resolve below takes
        # charge. Left running, bar 60 landed on top of the held chord 1.7s
        # before the last frame and put a 4 dB onset into the middle of the
        # closing title -- the loop audibly continuing under a card that has
        # already resolved.
        if at >= tail_from:
            break
        notes, bass = progression[b % 4]
        g = walk_arc(at / total, bloom_at, ramp_at)
    ...
    t.add(max(total * bloom_at - 1.0, 0), swell(1.8), 0.30)

    # The bed breathes about 9 dB inside every bar: `pad`'s release falls to
    # ZERO over the last 1.1s while the next chord needs 0.9s to come in. Under
    # narration nothing reveals that. Over the end card it did -- half a second
    # of near-silence at 159.4s and then the final chord arriving at full, so
    # the film sounded like it stopped and restarted on the CTA.
    #
    # One sustained chord across the tail fills it, which is also what the
    # moment wants: a resolve held under a closing title rather than a loop
    # still turning over. It is written LONGER than the film so that `pad`'s
    # own release lands past the last frame and `Track.add` clips it -- letting
    # it fade here would just rebuild the deflation this cue exists to remove.
    if tail_from < total:
        span = total - tail_from + 2.0
        g = walk_arc(0.999, bloom_at, ramp_at)
        chord(t, tail_from, ["A3", "C4", "E4"], span, pad, 1.25 * g)
        t.add(tail_from, sub(hz("A2"), span), 0.42 * g)
    return t.out(total)
...
def main():
    args = sys.argv[1:]
    # Where the narration stops, in seconds. Only `walkthrough` uses it, and
    # only the caller can know it -- it comes out of the timed manifest, after
    # recording has decided how long each beat really ran.
    bloom = None
    if "--bloom" in args:
        i = args.index("--bloom")
        bloom = float(args[i + 1])
        del args[i : i + 2]
    if len(args) not in (3, 4):
        print(
            "usage: score.py <launch|walkthrough> <out.wav> <seconds> [bpm] [--bloom <sec>]",
            file=sys.stderr,
        )
        return 1
    cue, out, seconds = args[0], args[1], float(args[2])
    bpm = float(args[3]) if len(args) == 4 else None
    if cue not in CUES:
    ...
    if bpm is not None:
        kw["bpm"] = bpm
    if bloom is not None and cue == "walkthrough":
        kw["bloom"] = bloom
    audio = CUES[cue](seconds, **kw)
